
module.exports = {
  env: {
    NEXT_PUBLIC_ENV: process.env.NEXT_PUBLIC_ENV,
    NEXT_PUBLIC_VERCEL_URL: process.env.NEXT_PUBLIC_VERCEL_URL,
    NEXT_PUBLIC_PROTOCOL: process.env.NEXT_PUBLIC_PROTOCOL,
    NEXT_PUBLIC_FIREBASE_DATABASE_URL: process.env.NEXT_PUBLIC_FIREBASE_DATABASE_URL,
    NEXT_PUBLIC_FIREBASE_PROJECT_ID: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID,
    NEXT_PUBLIC_FIREBASE_CLIENT_EMAIL: process.env.NEXT_PUBLIC_FIREBASE_CLIENT_EMAIL,
    NEXT_PUBLIC_FIREBASE_PRIVATE_KEY: process.env.NEXT_PUBLIC_FIREBASE_PRIVATE_KEY,
    NEXT_PUBLIC_FIREBASE_PRIVATE_KEY_ID: process.env.NEXT_PUBLIC_FIREBASE_PRIVATE_KEY_ID,
    NEXT_PUBLIC_FIREBASE_CLIENT_ID: process.env.NEXT_PUBLIC_FIREBASE_CLIENT_ID,
    NEXT_PUBLIC_AUTH_URI: process.env.NEXT_PUBLIC_AUTH_URI,
    S3_UPLOAD_KEY: process.env.S3_UPLOAD_KEY,
    S3_UPLOAD_SECRET: process.env.S3_UPLOAD_SECRET,
    S3_UPLOAD_REGION: process.env.S3_UPLOAD_REGION,
    S3_UPLOAD_BUCKET: process.env.S3_UPLOAD_BUCKET,
    NEXT_PUBLIC_S3_URL: process.env.NEXT_PUBLIC_S3_URL,
    POSTGRES_URL: process.env.POSTGRES_URL,
    GOOGLE_AUTH_CLIENT_ID: process.env.GOOGLE_AUTH_CLIENT_ID,
    GOOGLE_AUTH_CLIENT_SECRET: process.env.GOOGLE_AUTH_CLIENT_SECRET,
    LINKEDIN_CLIENT_ID: process.env.LINKEDIN_CLIENT_ID,
    LINKEDIN_CLIENT_SECRET: process.env.LINKEDIN_CLIENT_SECRET, 
    POSTGRES_URL: process.env.POSTGRES_URL,
    TWITTER_CLIENT_ID: process.env.TWITTER_CLIENT_ID,
    TWITTER_CLIENT_SECRET: process.env.TWITTER_CLIENT_SECRET,
    FIREBASE_APP_API_KEY: process.env.FIREBASE_APP_API_KEY,
    FIREBASE_APP_API_AUTHDOMAIN: process.env.FIREBASE_APP_API_AUTHDOMAIN,
    FIREBASE_APP_API_PROJECTID: process.env.FIREBASE_APP_API_PROJECTID,
    FIREBASE_APP_API_STORAGEBUCKET: process.env.FIREBASE_APP_API_STORAGEBUCKET,
    FIREBASE_APP_API_SENDERID: process.env.FIREBASE_APP_API_SENDERID,
    FIREBASE_APP_API_APPID: process.env.FIREBASE_APP_API_APPID,
    FIREBASE_APP_API_MEASUREMENTID: process.env.FIREBASE_APP_API_MEASUREMENTID

  },
  reactStrictMode: true,
  images: {
    domains: ['via.placeholder.com', 'media-exp3.licdn.com','qrme-vercel.s3.eu-central-1.amazonaws.com'],
  },
}